...
### 计算KS值
def ks_values(model,x,y,plot=True):#model 模型，x 特征列，y 目标列
    y = y.tolist()
    y_prob = model.predict_proba(x)
    data=pd.DataFrame(y)
    data['prob'] = y_prob[:, 1]
    #按照预测概率，划分100个区间
    prob_range_list = [data['prob'].min() + x * (data['prob'].max() - data['prob'].min()) / 50 for x in range(1, 51)]
    data_ks = pd.DataFrame(columns=[x for x in range(1, 51)])
    for k, i in enumerate(prob_range_list):
        k = k + 1
        t=pd.DataFrame(index=[0,1],columns=[0])
        bad_good_total = data[data['prob'] <= i].groupby(by=0).count()
        t = pd.concat([t,bad_good_total],axis=1)
        t = t.fillna(0)
        accumulative_ratio_i = t[['prob']]/ data.groupby(by=0).count().values
        data_ks[k] = accumulative_ratio_i
    data_ks = data_ks.T
    data_ks['prob_range'] = prob_range_list
    if plot:
        plt.plot(data_ks['prob_range'], data_ks[0], color='b', label='bad')
        plt.plot(data_ks['prob_range'], data_ks[1], color='r', label='good')
        plt.legend()
        plt.show()
    data_ks['ks'] = abs(data_ks[1] - data_ks[0])
    max_ks = data_ks['ks'].max()
    prob_value = data_ks[data_ks['ks'] == max_ks]['prob_range']

    return max_ks,prob_value
